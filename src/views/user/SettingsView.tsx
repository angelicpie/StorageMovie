...
export const SettingsView = () => {
  const { userName, setUserName, genrePrefs, setGenrePrefs } = useUserContext();
  const [value, setValue] = useState(userName);
  const [error, setError] = useState('');

  function handleMovieGenre(label: string) {
    const updated = genrePrefs.movies.includes(label)
      ? genrePrefs.movies.filter((g) => g !== label)
      : [...genrePrefs.movies, label];
    setGenrePrefs({ ...genrePrefs, movies: updated });
  }

  function handleTvGenre(label: string) {
    const updated = genrePrefs.tv.includes(label)
      ? genrePrefs.tv.filter((g) => g !== label)
      : [...genrePrefs.tv, label];
    setGenrePrefs({ ...genrePrefs, tv: updated });
  }
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              {movieGenreList.map((label) => {
                const isSelected = genrePrefs.movies.includes(label);
                return (
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-3 gap-2">
              {tvGenreList.map((label) => {
                const isSelected = genrePrefs.tv.includes(label);
                return (
